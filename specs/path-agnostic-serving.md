...
3. Drilled rollup `?path=` ✓ (`c95d068`). Server-side lenses — **blocked on the `by-user`/`by-team` index copies** (see §2.3 finding): build those in the pipeline first (they're the foundation for My files / communal / unclaimed / pinned), then wire the lens read + client. The `todo` lens can go first without them (mark-trie prune). Only then does `needFullTree` (hence `tree.json`) drop for lenses.
