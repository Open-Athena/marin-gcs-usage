...
2. Index re-layout: `a` column ✓ (`78b3bc8`), **footer-in-D1** ✓ (`c78a34e`), **8 k-row groups** ✓ (`c905031`); **remaining**: R2 copy (egress), and `/api/subtree` applying marks + claims per node (so the map's colors/rollups are claims-applied like `/users`, and the client stops fetching the whole ledger).
