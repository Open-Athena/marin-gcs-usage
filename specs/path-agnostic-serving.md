...
- **Row groups.** Tried 8 k rows (`78b3bc8`) — **reverted** (`9abbe7d`): over one monolithic 220 M-row file that's 27 k row groups, and parsing that footer on a cold CFN isolate exceeds the CPU budget (Cloudflare 1102 on fine-tier drills). Lesson: **group-shrink is coupled to depth-partitioning** — do them together. **Hive-partition by depth** (`path-index/depth=N/…`) so each per-depth file has few groups and a small footer; *then* 8 k groups within a partition are cheap. Back on 64 k rows/group meanwhile (the proven value).
...
2. Index re-layout: `a` column ✓ (`78b3bc8`); 8 k-row groups reverted (need partitioning first); **remaining**: depth partitions (then re-shrink groups), R2 copy, and `/api/subtree` applying marks + claims per node (so the map's colors/rollups are claims-applied like `/users`, and the client stops fetching the whole ledger).
