...
- ~~Artifact Registry cleanup~~ — done 2026-09-16: a policy on the shared `cloud-run-source-deploy` repository scoped to the `gcs-usage-snapshot` package (keep tagged, delete untagged > 14 days; 69 of 112 versions qualified). A dedicated repository was considered and skipped: the package scope gives the isolation, and moving would touch the build, `batch-submit.sh`, both live cron bodies, the `cw-s3` job scripts and the job SA's reader grant.
