...
**IaC (`~/c/oa/marin-gcs-usage/cf/`, NOT `ops/cf/gcs-usage` — the `cf-iac` memo's path is stale):** `Store` gains `r2_bucket`/`r2_location`/`r2_token_permission_groups[_ids]`; `CfnDashboard` adds children `r2` (`R2Bucket`) and `r2-token` (`ApiToken`, R2 bucket-item read+write, account-scoped; bucket-scoping is a follow-up once the bucket id exists), exporting `r2_bucket`, and as secrets `r2_s3_access_key_id` (= token id) and `r2_s3_secret_access_key` (= sha256 of the token value — R2's S3-credential derivation, so the token *is* the credential). cw-s3's `Store` now sets `kv_name="oa-cw-s3-usage-cache"` (the `CACHE_KV` tier) and `r2_bucket="oa-cw-s3-usage-index"`; `__main__.py` exports `cache_kv_id` + the R2 outputs. cw-s3's `Store` also sets `service_token` (a `ZeroTrustAccessServiceToken` + a `non_identity` policy on the Access app, precedence 2): the machine identity the job's 4b warm-cache stage presents as `CF-Access-Client-Id`/`-Secret`, exported as the secret outputs `service_token_client_id`/`service_token_client_secret` — so the whole credential set comes out of one `up`. Both stacks construct under Pulumi mocks (`cf/tmp/mock-test.py`; cw-s3 registers 11 children, gcs 7 — the first R2 draft did NOT: provider v6 takes the API token's `resources` as a JSON string, fixed 9/23 late). `cf/` is tracked on `gcs` since 5237f8f. Permission groups resolve by name via `get_api_token_permission_groups_list_output` — a read-scoped CI token can't list them, so CI `preview` needs `r2_token_permission_group_ids` set explicitly. **Backfill: Super Slurper (one-time bulk, console) + the publish step for steady state; Sippy not recommended** (needs a GCS SA key in CF, makes first reads cross-provider, redundant once every scan publishes) — reasoning in `cf/README.md`.

**User-gated checklist (in order):**
1. `cf/` (`gcs` branch, main checkout): `pulumi up` on the `cw-s3` stack. The first `up` both adopts the 6 live resources (Pages project, domain, CNAME, D1, Access app + policy — `importIds` are already in `Pulumi.cw-s3.yaml`) and creates the KV namespace, the R2 bucket, the R2 API token, and the warm-cache service token + its policy. The `CLOUDFLARE_API_TOKEN` in the env needs Pages / Access / DNS / D1 / Workers KV / R2 edit on the account, plus *User → API Tokens: Edit* (the R2 token is itself an API token). Then `pulumi config rm importIds` and an empty `pulumi preview`.
2. `site/wrangler.toml`: paste `cache_kv_id` into the `[[kv_namespaces]]` stanza and uncomment it (the edge cache's global tier, independent of R2). Do it in `[env.preview]` too — the dev stack shares the namespace.
3. Secrets, from `pulumi stack output --show-secrets`: `wrangler pages secret put STORE_ENDPOINT|STORE_ACCESS_KEY_ID|STORE_SECRET_ACCESS_KEY --project-name oa-cw-s3-usage` (production), the same three in the dashboard's Preview environment (the dev stack); into Secret Manager for the Batch job: `cw-s3-r2-endpoint`, `cw-s3-r2-access-key-id`, `cw-s3-r2-secret-access-key`, and `cw-s3-access-client-id` / `cw-s3-access-client-secret` (the `service_token_client_*` outputs — the 4b warm-cache stage's Access pair), each with the accessor grant to the job SA.
4. Backfill: Super Slurper over `snapshots/cw/` + `cw-l2/` GCS→R2 (or `for s in $(scans); do dt-cloud publish-r2 $s; done` on node `mgu`).
5. Wire `dt-cloud publish-r2 "$SNAP_ID"` into `job/cw-run.sh` (slot above) and rebuild `:cw` from `cw-s3-next`.
6. Flip on the dev stack first: uncomment `STORE_BUCKET`/`STORE_REGION` under `[env.preview.vars]`, `site/deploy --dev`, verify byte-equivalence against prod (`/data/cw/scans.json`, `/api/subtree`, `/api/diff`, `/api/path-index`, over-time) and re-run the walkDiff bench against `dev.oa-cw-s3-usage.pages.dev`; then mirror into production `[vars]` and `site/deploy`.
