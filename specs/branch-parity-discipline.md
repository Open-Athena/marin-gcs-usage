...
### 2026-09-16 (gcs-ward, `/cp cw-s3`) — the `cloud/` rename, mirrored

cw-s3 `39d56bf` (manifest §10), applied identically here: `gcs-usage/` → `cloud/` and `gcs_usage` → `dt_cloud` by `git mv`; distribution `marin-gcs-usage` → `dt-cloud`; entry point `dt-cloud`; every `gcs-usage <verb>` command string and CLI mention → `dt-cloud <verb>` (one regex pass, cw's lookaround rules). gcs-specific touchpoints: `job/run.sh` (30 command lines), `job/batch-submit.sh` untouched (names only infra ids — as does the live cron body, checked), `sheet-sync/` (Dockerfile, `sync.sh`, README), `.github/workflows/health.yml`, the sweep dispatch Function, `AGENTS.md` (the CLI docs: `pip install … #subdirectory=cloud`, `dt-cloud status|todo|mark`), `TokenModal` / `UserPage` / `SweepPage` prose. Kept, as on cw-s3: every infrastructure id containing the string (bucket, D1, Pages projects, image, SAs, secrets, Slack channels, the `gcs-usage:*` prefs keys and use-kbd store key), the User-Agent strings (`gcs-usage-cli/1.0` etc. — server-side allow-lists may key on them), `src/disk_tree` / `tests/` / `packages/` (parity surfaces with upstream), `specs/` history. `cloud/uv.lock` relocked. The job image and the sheet-sync image need a rebuild to pick up the new entry point; the old images are self-consistent (old `run.sh` + old CLI) until then.

### 2026-09-16 (gcs-ward) — the union's generalizations (manifest §11), gcs = union + A

`git diff gcs denovo/cw-s3-gcs-2026-09-16 -- . ':!specs' ':!pnpm-lock.yaml'` (tip `59433de`, 44 files) applied as one patch, then adapted: `build-dist.yml` kept gcs's (the union carried cw's `main`-branch trigger and dropped `on_source_rewrite: preserve`); the login wall's restriction sentence / sign-in label / how-to moved onto the store row (`Store.wall`) instead of taking cw's "Open Athena members" copy; `ChildrenTable`'s `read` / `owner(s)` columns are gated by the *scan* (`readAxis` / `ownerAxis` from App's `readRange` / `hasAttr`) with the union's per-page fallback, so a directory whose children are all unread or unowned still shows the column as dashes; `warm.warm`'s docstring and a `pip install ./marin` comment fixed. Everything else as-is with gcs's defaults (`EDGE_TRUSTED` unset, `BASE_SCOPE`/`ROOT_LABEL`/`D1_DB_*`/`INDEX_VARIANTS`/`WARM_PATHS` defaulting to gcs's values, `JOB=run.sh`, `site/deploy`/`cf-status` reading `wrangler.toml`, `dev` reading `devPort`). Checkpoint: site + functions `tsc`, 24 vitest, 184 py; CIC on the dev server (wall copy, home table columns, title, `marin GCS` crumb, API probes). Spec: `specs/gcs-toward-union.md` §1 done; `cw-s3..gcs` should now be the deployment deltas plus whatever cw-s3 landed after `73183b3` (the fast-forward + the D1 lineage guard `afb9027`), to be assessed on the next `/read`.

### 2026-09-16 (gcs, de novo) — bucket lifecycle rules for the GCS fleet

`specs/gcs-toward-union.md` §2, built de novo against cw's shape rather than ported: `cloud/src/dt_cloud/lifecycle.py` keeps cw's S3 functions verbatim (`normalize`/`gc_rule`/`pull`/`diff`/`push`, ID-keyed) and adds a GCS backend (`pull_gcs`/`diff_gcs`/`push_gcs` on the storage client — rules are anonymous, so identity = canonical content and `diff_gcs` has no *changed*), scheme dispatch (`gs://` → GCS), and `pull_many` for the fleet's `{bucket: rules}` snapshot. CLI `dt-cloud lifecycle pull|diff|push|gc-rule` with a repeatable `-b` (several → the keyed map); `job/run.sh` writes `<scan>/lifecycle.json` for the six buckets after the snapshot publish (the job SA's `legacyBucketReader` carries `storage.buckets.get`); `job/lifecycle/<bucket>.json` × 6 tracked (`diff` exit 0 against live). Site: `lifecycle.ts` = cw's plus `fromGcs` (age/prefix/noncurrent/abort-MPU mapped, the rest worded into `Extra`, a content ID; `displayId` drops the prefix term the table shows), `normalizeSnapshot` (list or map), `groupRows` (fleet-wide rules once, with a buckets column), numeric-aware ID sort; `LifecycleFold` = cw's plus the buckets column and prefix clipping; 13 new vitest cases, 14 py. `Store.lifecycle = 'job/lifecycle/'` mounts the fold on the home page. Published the pulled snapshot for 9/15 and 9/16 so the fold has data before the first job run. cw-ward: `lifecycle.ts`/`LifecycleFold.tsx`/`lifecycle.py` are additive over cw's — the union takes them as the shared implementation (GCS adapter beside CAIOS's).

### 2026-09-17 (gcs-ward, targeted CP) — secrets whitespace-stripped at every read

cw-s3 `0858d6d`, cherry-picked: `cloud/src/dt_cloud/secrets.py` (`env_secret`/`secret`) behind every token/webhook read in `cli.py`/`mark.py`/`index_footer.py`, and `job/run.sh` strips the Batch secretVariables in an xtrace-off block before boto/curl/wrangler see them. Resolved: the conflict block was cw's `compare`/`cw-digest` commands (B delta, dropped); `job/cw-run.sh` doesn't exist here (dropped). Occasion: the GCS job's Discord digest failed 9/16–9/17 on `curl exit 3` — the `gcs-usage-discord-webhook` secret version carried a trailing newline; cw-s3 added a clean version 2 the same morning. Not a cursor advance — cw-s3's commits after `39d56bf` other than this one are still to triage.
