...
**P4 — Cutover.** Both paths verified on the local stack 2026-09-23 (Google on localhost:3257; emailed link over the tailnet from a phone, to an OA address). Flip DONE in code: `signInUrl()` → the app's own `/signin?next=` page (the wall as an ungated route), so nothing links to `/auth/sso` any more; the request-access form + how-to prose fold behind a disclosure that a `?denied=` bounce unfolds. Still pending, after the user-run `site/deploy` and a prod re-check of both paths: remove `functions/auth/sso.ts` + the `ACCESS_*` vars (second deploy), then delete the ZT Access app + policy. Rollback until then = revert `signInUrl` to `/auth/sso`.
