...
|---|---|---|---|---|---|---|
| 1 | ~~**`amap` → SQL**~~ **done 2026-09-06** (`654eb00`): the dict's only consumer was the `tree.json` decoration, and `tree.json` is no longer built (`view-serving.md`); `a` reaches the index and age strata via SQL | −19 GiB, and the only monotonic term is gone | −1 min | 0 | — | the next daily is the measurement; if the peak lands near 105 GiB the scheduler can go back to highmem-16 |
