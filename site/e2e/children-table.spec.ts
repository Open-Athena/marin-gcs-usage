...
// "Rendered fewer hooks than expected" and blank the page (2026-09-11: marks
// → keep on a band whose children all inherit sweep).
test('emptying the children table via the mark axis renders its note, not a crash', async ({ page }) => {
  ...
  await page.evaluate(() => {
    history.pushState(null, '', '/marin-us-east5/grug?o=kaiyue&k=k')
    window.dispatchEvent(new PopStateEvent('popstate'))
  })
  await expect(page.locator('#tbl .tab-note')).toHaveText('No prefix under this view is keep.')
