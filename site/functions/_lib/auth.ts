...
export interface Env {
  ...
  /** Generalized index-store seam (the base's specs/union-of-roots.md; cw's
   *  specs/r2-serving-migration.md). Point every store read at any
   *  S3-compatible store; unset falls back to the GCS defaults + `GCS_HMAC_*`
   *  so the GCS-backed deploy is unchanged until flipped. */
