  ...
  return (
    <section className="mark-tabs">
      <div className="tabrow">
        {tabs.map(([t, label]) => (
          <button key={t} type="button" className={tab === t ? 'on' : ''} onClick={() => { setTab(t); setLimit(PAGE) }}>
            {label}
          </button>
        ))}
      </div>
        ...
        <>
          ...
          <table className="worklist">
            ...
            <tbody>
              ...
              {!rows.length && <tr><td colSpan={6}><em>nothing above the size threshold for this lens</em></td></tr>}
            </tbody>
          </table>
          {rows.length > limit && (
...
function Row({ r, idx, lost, fmtBytes, onMark, onClaim }: {
  r: SweepRow
  idx: MarkIndex
  lost: boolean
