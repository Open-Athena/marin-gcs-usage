...
// Client-side diff for arbitrary scan pairs: align two pixel-budget
// `/api/subtree` trees by child name and emit the same `DiffData` rows the
// precomputed `diff.json` carries. The precomputed N-1→N pair comes from the
// batch job's exact best-first walk over the full path index; this aligner is
// the interactive fallback when the viewer picks a different "before" scan —
// each side's tree is that date's own budget fold, so small dirs may hide
// inside "(other)" tiles, which are merged into one aligned pseudo-child per
// parent (their combined delta stays truthful even when membership shifted).
