 ...
 * "pin" UX re-implemented across pltly (useTraceHighlight/useLegendPin),
 * ctbk's Leaflet StationMap, and hbt's GeoSankey, with the hard-won fixes
 * from those implementations baked in:
 *
...
export function useHoverPin<K>({ excludeRefs = [], excludeSelectors = [] }: HoverPinOpts = {}): HoverPin<K> {
  ...
  // key whose hover is suppressed after an unpin re-click, until pointer leaves
  ...
  useEffect(() => {
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps -- refs/selectors are stable per callsite
  }, [pinned])
