...
R2 (no cross-provider round trips, no egress on reads). The ingest keeps
building on GCP Batch against GCS; this is the pipeline's final "publish to the
serving cloud" stage (specs/r2-serving-migration.md §3).
...
def served_prefixes(scan: str, subdir: str = "cw", layer2_root: str = LAYER2_ROOT) -> list[str]:
    """The key prefixes that make up one scan's served subset (spec §2):
    the published snapshot JSONs and the layer-2 dir, which holds both the
    canonical parquets the file browser opens and `index/<gen>/` — every
    tier + `.groups.json` manifest + age pyramid the index reader serves."""
    return [f"snapshots/{subdir}/{scan}/", f"{layer2_root}/{scan}/"]
...
def publish(
    ...
    *,
    ...
    prefixes = prefixes or served_prefixes(scan, subdir)
