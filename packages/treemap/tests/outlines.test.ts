...
describe('unionOutline: float-epsilon seams', () => {
  it('treats a seam that agrees only to float epsilon as one line', () => {
    // Squarify-style neighbors: the left rect's `x + w` lands at 100 + ulp.
    const a = { x: 0, y: 0, w: 100 + 1e-9, h: 10 }
    const b = { x: 100, y: 0, w: 50, h: 10 }
    const segs = unionOutline([a, b])
    const xs = segs.filter(s => s.nx !== 0).map(s => s.x1).sort((p, q) => p - q)
    expect(xs).toEqual([0, 150])
    expect(segs).toHaveLength(4)
  })

  it('drops zero-length segments', () => {
    const segs = unionOutline([{ x: 0, y: 0, w: 10, h: 10 }, { x: 10, y: 0, w: 10, h: 10 }])
