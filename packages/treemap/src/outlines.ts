...
export function unionOutline(rects: Rect[]): OutlineSeg[] {
  ...
  // Squarify hands neighbors seams that agree only to float epsilon (one
  // rect's `x + w` vs. the next one's `x`). Bucketed by raw value they land on
  // two different lines, each bordered on one side only, and the shared seam
  // strokes twice — the fence this overlay exists to remove. Snap every
  // coordinate to 1/8 px first; that's below what the stroke can show.
  ...
  for (const r of rects) {
    const span: Iv = [q(r.y), q(r.y + r.h)]
    atX(q(r.x)).right.push(span)
    atX(q(r.x + r.w)).left.push(span)
  }
  for (const [x, { left, right }] of byX) {
    for (const { lo, hi, side } of symDiff(mergeIntervals(left), mergeIntervals(right))) {
      if (hi > lo) segs.push({ x1: x, y1: lo, x2: x, y2: hi, nx: side === 'a' ? -1 : 1, ny: 0 })
    }
  }
  ...
  for (const r of rects) {
    const span: Iv = [q(r.x), q(r.x + r.w)]
    atY(q(r.y)).below.push(span)
    atY(q(r.y + r.h)).above.push(span)
  }
  for (const [y, { above, below }] of byY) {
    for (const { lo, hi, side } of symDiff(mergeIntervals(above), mergeIntervals(below))) {
      if (hi > lo) segs.push({ x1: lo, y1: y, x2: hi, y2: y, nx: 0, ny: side === 'a' ? -1 : 1 })
    }
  }
