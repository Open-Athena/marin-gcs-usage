...
export interface TreemapProps<T> {
  ...
  /**
   * Where the hover / pin tip renders. `float` (default) is a fixed tip
   * anchored near the pointer. `dock` renders it into a fixed panel below the
   * footer row, so moving through a lineage updates one panel in place instead
   * of a tip that chases the pointer (and covers the cells and controls under
   * it). Pinning still works — the panel stays, its × clears the pin — and a
   * phone, which has no hover, taps to pin into the panel. The panel holds a
...
export function Treemap<T>({
  ...
  // Backspace pops the drill stack — unless another listener already consumed
  // the key (a hotkey layer clearing a selection, closing a modal…). Escape is
  // NOT a drill-up (it reads as "dismiss", and popping the whole view on Esc
  // surprised people); it stays reserved for unpinning the tip / closing
  // overlays. Listens on `window`, the end of the bubbling path, so a hotkey
  // layer on `document` or `window` gets to preventDefault first.
  useEffect(() => {
