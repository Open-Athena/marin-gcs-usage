...
export interface TimeSeriesProps<T> {
   ...
   *  as a band; with `onBrush`, dragging inside it slides the window (same
   *  number of points) and dragging outside brushes a new one. Drawn
...
export function TimeSeries<T>({
  ...
  // A drag that starts inside the shown window SLIDES it (same width in
  // points, clamped to the data) instead of brushing a new one; a drag that
  // starts outside brushes as before. Held in point indices so the window
  // keeps its scan count while the x spacing varies.
